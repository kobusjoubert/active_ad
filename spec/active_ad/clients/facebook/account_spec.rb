...
RSpec.describe ActiveAd::Facebook::Account do
  ...
  let(:stub_create_100_with_attributes) {
    stub_request(:post, "#{client.base_url}/100/adaccount").with(body:
      ...
      id: 'act_101', name: 'Account Name', currency: 'USD', timezone_id: 0, partner: 'NONE', media_agency: 'NONE', end_advertiser: 'NONE'
    }.to_json)
  }
  ...
  let(:stub_update_101) {
    stub_request(:post, "#{client.base_url}/act_101").with(body:
      ...
      id: 'act_101', name: 'New Account Name'
    }.to_json)
  }
  ...
  describe '.create!' do
    ...
    it 'raises an exception when not created' do
      stub_create_900
      expect { described_class.create!(business_id: '900', validate: false) }.to raise_error(ActiveAd::RecordInvalid)
    end
  ...
  describe '#save!' do
    ...
    it 'raises an exception when not saved' do
      stub_update_901
      account_901.name = 'New Account Name'
      expect { account_901.save! }.to raise_error(ActiveAd::RecordInvalid)
    end
  end
  ...
  describe '#update!' do
    ...
    it 'raise an exception when not updated' do
      stub_update_901
      expect { account_901.update!(name: 'New Account Name') }.to raise_error(ActiveAd::RecordInvalid)
    end
  end
