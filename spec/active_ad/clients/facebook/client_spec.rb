...
RSpec.describe ActiveAd::Facebook::Client do
  let(:client) { described_class.new(access_token: 'secret_access_token', client_id: 'client_100', client_secret: '1a2b3c') }
  ...
  describe '#login' do
    it 'retrieves and sets a long lived access_token in exchange for a short lived access_token' do
      stub_request(:get, "#{described_class.base_url}/oauth/access_token").with(query: {
        client_id: 'client_100', client_secret: '1a2b3c', grant_type: 'fb_exchange_token', fb_exchange_token: 'secret_short_access_token'
  ...
  describe '#refresh_token' do
    it 'retrieves and sets a refreshed long lived access_token in exchange for the current long lived access_token' do
      stub_request(:get, "#{described_class.base_url}/oauth/access_token").with(query: {
        client_id: 'client_100', client_secret: '1a2b3c', grant_type: 'fb_exchange_token', fb_exchange_token: 'secret_access_token'
      }).to_return(status: 200, body: {
  ...
  describe '#user' do
    ...
    before(:each) do
      stub_request(:get, "#{client.base_url}/me").with(query:
        hash_including(access_token: 'secret_access_token')
      ).to_return(status: 200, body: {
        id: '100', name: 'User Name'
      }.to_json)
    end
