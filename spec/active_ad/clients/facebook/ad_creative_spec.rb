...
RSpec.describe ActiveAd::Facebook::AdCreative do
  ...
  let(:ad_creative_101) { described_class.new(id: '101', stale: true) }
  let(:ad_creative_801) { described_class.new(id: '801', stale: true) }
  let(:ad_creative_901) { described_class.new(id: '901', stale: true) }
  ...
  # POST create_request.
  let(:stub_create_100) {
    stub_request(:post, "#{client.base_url}/act_100/adcreatives").with(body:
      hash_including(access_token: 'secret_access_token')
    ).to_return(status: 200, body: {
      id: '101'
    }.to_json)
  ...
  let(:stub_create_100_with_attributes) {
    stub_request(:post, "#{client.base_url}/act_100/adcreatives").with(body:
      hash_including(access_token: 'secret_access_token', name: 'Ad Creative Name', object_story_spec: { page_id: '200' })
    ).to_return(status: 200, body: {
      id: '101'
    }.to_json)
  }
  ...
  let(:stub_create_900) {
    stub_request(:post, "#{client.base_url}/act_900/adcreatives").with(body:
      hash_including(access_token: 'secret_access_token')
  ...
  let(:stub_update_101) {
    stub_request(:post, "#{client.base_url}/101").with(body:
      hash_including(access_token: 'secret_access_token', name: 'New Ad Creative Name')
    ).to_return(status: 200, body: {
      success: true
    }.to_json)
  }
  ...
  describe '.find' do
    it 'returns an object when found' do
      stub_read_101
      expect(described_class.find('101')).to be_an_instance_of(ActiveAd::Facebook::AdCreative)
    end
  ...
  describe '.create' do
    ...
    it 'does not allow an account_id of 0' do
      object = described_class.create(account_id: '0', name: 'Ad Creative Name', object_story_spec: { page_id: '200' })
      expect(object.attributes.compact).to be_empty
    end
    ...
    it 'sets no attributes when no object_story_spec supplied' do
      object = described_class.create(account_id: '100', name: 'Ad Creative Name')
      expect(object.attributes.compact).to be_empty
    end

    it 'needs all the required attributes to be supplied' do
      stub_create_100_with_attributes
      object = described_class.create(account_id: '100', name: 'Ad Creative Name', object_story_spec: { page_id: '200' })
  ...
  describe '.create!' do
    ...
    it 'raises an exception with account_id of 0' do
      expect {
        described_class.create!(account_id: '0', name: 'Ad Creative Name', object_story_spec: { page_id: '200' })
    ...
    it 'raises an exception with no object_story_spec supplied' do
      expect {
        described_class.create!(account_id: '100', name: 'Ad Creative Name')
      }.to raise_error(ActiveAd::RecordInvalid)
    end
