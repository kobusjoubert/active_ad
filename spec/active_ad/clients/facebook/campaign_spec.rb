...
RSpec.describe ActiveAd::Facebook::Campaign do
  ...
  describe '#new_record?' do
    ...
    it 'is not a new record after update' do
      stub_update_101
      campaign_101.update(name: 'New Campaign Name')
      expect(campaign_101.new_record?).to be(false)
    end
  ...
  describe '#valid?' do
    ...
    it 'is valid if no name is present when updating an existing record' do
      campaign_101.name = nil
      expect(campaign_101.valid?).to be(true)
    end
  end
  ...
  describe '#save' do
    it 'returns true when saved' do
      stub_update_101
      campaign_101.name = 'New Campaign Name'
      expect(campaign_101.save).to be(true)
    end

    it 'returns false when not saved' do
      stub_update_901
      campaign_901.name = 'New Campaign Name'
      expect(campaign_901.save).to be(false)
    end
  end
  ...
  describe '#save!' do
    ...
    it 'raises an exception when not saved' do
      stub_update_901
      campaign_901.name = 'New Campaign Name'
      expect { campaign_901.save! }.to raise_error(ActiveAd::RecordNotSaved)
    end
  end
  ...
  describe '#update' do
    it 'returns true when updated' do
      stub_update_101
      expect(campaign_101.update(name: 'New Campaign Name')).to be(true)
    end

    it 'returns false when not updated' do
      stub_update_901
      expect(campaign_901.update(name: 'New Campaign Name')).to be(false)
    end
  end
  ...
  describe '#update!' do
    ...
    it 'raise an exception when not updated' do
      stub_update_901
      expect { campaign_901.update!(name: 'New Campaign Name') }.to raise_error(ActiveAd::RecordNotSaved)
    end
  end
  ...
  describe '#destroy' do
    it 'returns true when deleted' do
      stub_delete_101
      expect(campaign_101.destroy).to be(true)
    end

    it 'returns false when not deleted' do
      stub_delete_801
      expect(campaign_801.destroy).to be(false)
    end

    it 'returns false on a bad request' do
      stub_delete_901
      expect(campaign_901.destroy).to be(false)
    end
  end
  ...
  describe '#destroy!' do
    ...
    it 'raises an exception when not deleted' do
      stub_delete_801
      expect { campaign_801.destroy! }.to raise_error(ActiveAd::RecordNotDeleted)
    end

    it 'raises an exception on a bad request' do
      stub_delete_901
      expect { campaign_901.destroy! }.to raise_error(ActiveAd::RecordNotDeleted)
    end
  end
  ...
  describe '#account' do
    ...
    it 'returns the object type' do
      campaign_101.account_id = '100'
      expect(campaign_101.account.class).to be(ActiveAd::Facebook::Account)
    end

    it 'returns the object' do
      campaign_101.account_id = '100'
      expect(campaign_101.account.name).to eq('Account Name')
    end
  end
