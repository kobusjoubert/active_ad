...
RSpec.describe ActiveAd::Facebook::User do
  ...
  let(:user_101) { described_class.new(id: '101', stale: true) }
  let(:user_901) { described_class.new(id: '901', stale: true) }
  ...
  describe '.find' do
    it 'returns an object when found' do
      stub_read_101
      expect(described_class.find('101')).to be_an_instance_of(ActiveAd::Facebook::User)
    end
  ...
  describe '#businesses' do
    ...
    it 'returns the objects when invoked' do
      stub_request(:get, "#{client.base_url}/#{user_101.id}/businesses").with(query:
        hash_including(access_token: 'secret_access_token')
      ).to_return(status: 200, body: {
        data: [
